...
# The EXACT columns the model was trained on, in the EXACT order.
# Must match what pd.get_dummies(drop_first=True) produced in the notebook.
TRAINING_COLUMNS = [
...
def preprocess_input(raw_input: dict) -> np.ndarray:
    """
    Convert raw user input into a model-ready numpy array.
    ...
    Step 4 — Add any missing dummy columns as 0
    Step 5 — Reorder to match the exact training column order
    Step 6 — Scale the four numerical columns
    Step 7 — Convert to a plain numpy array (strips column names)
              This prevents the scikit-learn feature name mismatch error
              that occurs when a model trained on arrays receives a DataFrame
    """
    # Step 1
    ...
    # Step 5 — reorder columns to exact training order
    df = df[TRAINING_COLUMNS]

    # Step 6 — scale numerical features
    df[NUMERICAL_COLS] = scaler.transform(df[NUMERICAL_COLS])

    # Step 7 — convert to numpy array (eliminates feature name mismatch)
    return df.to_numpy()
...
if predict_clicked:
    ...
    # Preprocess — returns numpy array (no feature name mismatch)
    X_input    = preprocess_input(raw_input)
    prediction = model.predict(X_input)[0]
    prob_high  = model.predict_proba(X_input)[0][1]
    prob_low   = model.predict_proba(X_input)[0][0]
